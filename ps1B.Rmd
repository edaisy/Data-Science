...
ggplot(data = MLB_teams) + 
  geom_point(aes(x = payroll, y = WPct)) + geom_smooth(aes(x = payroll, y = WPct), method = "loess", se = FALSE) +
  labs(title = "Win Percentage based on Total Payroll") + xlab("Sum of Player Salaries ($)") + ylab("Percentage of Games Won")
MLB_teams %>%
...
library(nasaweather)
glimpse(storms)
ggplot(data = storms) + geom_path(aes(x = long, y = lat, color = name)) + scale_color_discrete(guide="none") + facet_wrap(~year, nrow = 2)
```
