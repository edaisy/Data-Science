...
word_frequencies %>%
  with(wordcloud(words = word, freq = n, max.words=100, random.order=T, scale=c(2,0.3), rot.per=.15, colors = pal, family="sans"))
```
...
> ANSWER: Because the tf-idf will not be skewed by the frequency of stop words, considering that it controls for how frequently the word shows up in the dataset as a whole.
...
> ANSWER: The word "slick" has a rating of 2, indicating a somewhat positive connotation, but in some situations slick can mean dangerous and icy. For example, "beware, the steps are slick tonight".

```{r}
afinn_lexicon <- get_sentiments("afinn")
slick_lexicon <- afinn_lexicon %>%
  filter(word == "slick")
...
> ANSWER: Each row represents a word and an associated sentiment (fear, negative, sadness for abandon), thus there can be multiple rows per word unlike in the AFINN lexicon
...
> ANSWER: Words can have multiple definitions, making it hard to definitively ascribe certain sentiments to a given word across contexts.

- The lexicons are based on unigrams.  Do you see any disadvantages of basing the sentiment on single words?

> ANSWER: Words can have different sentiments depending on their contexts, making it hard to ascribe sentiments based on a single word taken out of context.
...
> ANSWER: 78% of the unigrams in the corpora are not scored by this lexicon.
...
```{r}
nrc_found <- word_frequencies %>%
  anti_join(nrc_missed_words, by ="word") %>%
  inner_join(nrc_lexicon, by = "word") %>%
  filter(sentiment %in% c("anger", "anticipation", "fear", "joy", "surprise", "trust")) %>%
  arrange(sentiment, desc(n)) %>%
  group_by(sentiment) %>%
  slice(1:10)
ggplot(data=nrc_found, aes(x = reorder(word,n)
                           , y = n
                           , fill = sentiment)) +
  geom_col(show.legend = FALSE) +
...
How might one summarize the sentiment of this corpus using the NRC lexicon?

> ANSWER: I would call this corpus highly anticipatory, with a heary sprinkling of joy and fear, a dash of trust and surprise, and pinch of anger.
