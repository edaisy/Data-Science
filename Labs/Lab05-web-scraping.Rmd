...
> ANSWER: The URL for the page that contains poem text is made with the normal wikipedia intro followed by the title of the poem with underscores in place of spaces.
...
```{r}
new_table <- table %>%
  mutate(titles = str_replace_all(first_line_often_used_as_title, " ", "_")) %>%
  mutate(url = paste0("https://en.wikisource.org/wiki/", titles))
```
...
# all the poems
poem_table_all <- new_table %>%
  rename(first_line = first_line_often_used_as_title)
