...
# print the plot 
g + geom_text(aes(label = country, color = net_users), size = 3) + theme(legend.position = "top") + 
  ylab("Gross Domestic Product (GDP)") + 
  xlab("% of GDP spent on education")
```
...
> ANSWER: This function puts the medical procedures in order along the x-axis based the mean charge for the procedure (mean_charge) from lowest to highest.
...
# print the plot
p + geom_bar(fill = "gray", stat = "identity") + ylab("Statewide Average Charges ($)") + xlab("Medical Procedure (DRG)") + theme(axis.text.x = element_text(angle = 90, hjust = 1))
```
...
> ANSWER: The theme line rotates the x-axis labels for each numerical identifier of medical procedure so that they are vertical (perpendicular to the x-axis) and don't overlap as much with each other.
...
# print the plot
p + geom_bar(fill = "gray", stat = "identity") + ylab("Statewide Average Charges ($)") + xlab("Medical Procedure (DRG)")
```
...
> ANSWER: Why add all of these aspects of the name_plot (axis labels, geom_line) individually? Why not add them all in one line at the end?
...
name_plot <- name_plot + geom_bar(stat = "identity", colour = "white", fill = "#008fd5", aes(y = ifelse(year == median_yob, est_alive_today / 1000, 0)))
name_plot <- name_plot %+% filter(BabynamesDist, name == "Evan", sex == "M")
name_plot
name_plot <- name_plot %+% filter(BabynamesDist, name == "Jordan")
