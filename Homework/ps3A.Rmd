...
> ANSWER: What is the purpose of reactiveValues(), and how does it work?
What is the purpose of observeEvent() and how does it interact with reactiveValues()?
...
ui <- fluidPage(
  sliderInput(inputId = "num", 
    label = "Choose a number", 
    value = 25, min = 1, max = 100),
  actionButton(inputId = "gogo", 
    label = "Do it"),
  plotOutput("hist")
)

server <- function(input, output) {
  data <- eventReactive(input$gogo, {
    rnorm(input$num) 
  })
...
ui <- fluidPage(
  fluidRow(
   column(1),
   column(7, sliderInput(inputId = "num", 
     label = "Choose a number", 
     value = 25, min = 1, max = 100))
  ),
  fluidRow(
