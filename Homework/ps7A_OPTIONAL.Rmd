...
author: "Evan Daisy"
date: "OPTIONAL (will not be submitted)"
output:
...
Section 9.1.2 walks through an example of how k-means clustering can identify genuine patterns in data -- in this case, clustering cities into continental groups merely based on city location (latitude and longitude coordinates).  The code below functions similarly to the code provided in this section of the textbook, but uses slightly different syntax.  It also adds the centroid locations to a reproduction of Figure 9.4.
...
> ANSWER: 726 cities were identified as belonging to cluster 1, and this cluster represents the big cities of the Middle East, India, and central Asia.
...
> ANSWER: Now there are 1466 cities in cluster 1, and this cluster now consists of the big cities in Europe, the Middle East, Western Asia, and Africa.

```{r}
set.seed(15)
tm_out <- kmeans(BigCities, centers = 3, nstart = 20)
tm_out$size
tm_out$centers

BigCities_tm <- BigCities %>%
  mutate(cluster = as.character(tm_out$cluster))

ggplot(data = BigCities_tm, aes(x = longitude, y = latitude)) + 
  geom_point(aes(color = cluster), alpha = 0.5) +
  # add centroids to plot
  geom_point(data = as.data.frame(tm_out$centers), shape = "X", size = 3)
```
...
> ANSWER: There are now 593 cities in cluster 1, and this cluster consists of the big cities of India and Central Asia.

```{r}
set.seed(15)
fm_out <- kmeans(BigCities, centers = 15, nstart = 20)
fm_out$size
fm_out$centers

BigCities_fm <- BigCities %>%
  mutate(cluster = as.character(fm_out$cluster))

ggplot(data = BigCities_fm, aes(x = longitude, y = latitude)) + 
  geom_point(aes(color = cluster), alpha = 0.5) +
  # add centroids to plot
  geom_point(data = as.data.frame(fm_out$centers), shape = "X", size = 3)
```
