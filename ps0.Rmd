...
> RESPONSE: This indicates that if we combine all of the datasets included in the datasaurus dozen, ignoring the different x and y variables these datasets likely have, we get a correlation of -0.06447185, meaning that if we tried to fit a linear pattern to this data, we would observe a very weakly negative correlation (as x increases, y decreases slightly). As it is, we have no idea what the different variables are for the different datasets and cannot lump them together like this, so the correlation coefficient means nothing.
