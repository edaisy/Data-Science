...
> RESPONSE: This data set has 1846 rows and 3 columns. The three variables are dataset, x, and y.
...
> RESPONSE: This correlation coefficient of -0.06447185 means that if we tried to fit a linear pattern to this data, we would observe a very weakly negative correlation (as x increases, y decreases slightly). As it is, we have no idea what the variables are for this data set and whether a linear model would be appropriate, so we can't say much using the correlation coefficient.
...
> RESPONSE: I notice that the shape of this scatterplot is remarkably like a T-rex, indicating a very saurian relationship between x and y.
...
> RESPONSE: This correlation coefficient is very similar to the one we calculated for the dino dataset, indicating a similarly weak negative correlation (if we assume a linear model is appropriate, which it probably isn't).
...
> RESPONSE: The plot does not look the same! It looks like a star!
...
> RESPONSE: This correlation coefficient is also very similar to those calculated for the other two datasets.
...
> RESPONSE: It doesn't! It looks like an ellipse with a few extra points, or something.
...
> RESPONSE: Although all of the correlation coefficients are about the same, the graphs look completely different. This indicates to me that the correlation coefficient is not a good summary of these datasets (because they're not linear).
